...
The demo application is based on Spring Boot. So its possible to run the Demo as Java Application right out of eclipse, there is not servlet container needed as Spring Boot embeds a Tomcat.
...
5. Set `-Dprofile=dev` as VM argument. This ensures that source code panel is correctly filled while developing
